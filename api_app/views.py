...
@permission_classes([IsAuthenticated])
def Data(request):
    corporate_list = CorporatesList.objects.filter(status=1).values_list('corpo_name', 'corporate_type_id').order_by('corpo_name')
...
def SiteVisitFrom(request):
    ...
    if request.method == 'POST':  
        ...
        occupation_type = request.data.get('occupation_type') 
        ...
        try: 
            ...
            if customer_id:     
                ...
                if entry_exisit: 
                    ...
                    entry_exisit.occupation_type = occupation_type
                ...
                data = SiteVisit.objects.create( sales_name = sales_man, Remark = Remark, reference= reference, visit_type = visit_type,
                    ...
                    , marital_status= marital_status , revisit = revisit, occupation_type = occupation_type 
                )        
                data.save()
                return JsonResponse({'success':'Site Visit Form submited successfully'}, status = status.HTTP_200_OK)
...
def Home_Visit(request):
    ...
    if request.method == 'POST':
        try:       
            ...
            ext = 'webp'
            new_filename = f"{slugify(name)}{Today_date}{slugify(Visit_location)}_{slugify(currentTime)}.{ext}"
